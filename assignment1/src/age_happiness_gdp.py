...
AGE = "UN Population Division (Median Age) (2017) (years)"

# use pandas to read csv
df_gdp = pd.read_csv("../res/gdp.csv")
df_age = pd.read_csv("../res/median-age.csv")
...
age_entries = df_age[(df_age["Year"] == SELECTED_YEAR)]
...
merged_entries = pd.merge(gdp_entries, age_entries, on=["Code", "Year", "Entity"])
...
for df, color, label in inputs:
    ax.scatter(
        df[AGE], df[HAPPINESS], c=color, s=75, label=label, alpha=0.8, edgecolors="none"
    )

ax.legend()

plt.title("Median age and happiness (2010)")

plt.xlabel(AGE)
plt.ylabel(HAPPINESS)
plt.savefig("../fig/age_happiness_gdp-2010.png")
plt.show()
