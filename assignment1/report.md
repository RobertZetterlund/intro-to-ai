...
- We define the intersection between `HG` and `LE` to be `Z`. The percentage of entities within `LE` and `HG` (excluding the intersection) are denoted `X` and `Y` respectively.
